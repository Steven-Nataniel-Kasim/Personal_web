...
export default function Home() {
  ...
  return <>
    ...
    <main>
      ...
      <section className="about section-shell" id="about"><div><p className="section-index mono">02 / ABOUT</p><p className="side-label mono">INPUT → MODEL → INFERENCE</p></div><Reveal className="about-copy"><h2>Curiosity, made physical.</h2><p>I&apos;m Steven, an Informatics student fascinated by what happens when artificial intelligence leaves the cloud and moves closer to hardware.</p><p>My work and experiments span computer vision, edge AI, FPGA systems, digital architecture, and—more recently—the hardware foundations of intelligent computing.</p><p>My long-term curiosity goes deeper: understanding intelligence across artificial and biological systems.</p></Reveal></section>
      ...
      <section className="projects" id="work"><div className="section-heading"><p className="section-index mono">04 / SELECTED EXPERIMENTS</p><h2>WORK IN<br />PROGRESS.</h2><p>Projects are presented as experiments—not finished monuments. Select a row for the technical direction.</p></div><div className="project-list">{projects.map((project, index) => <article className={`project ${projectOpen === index ? "open" : ""}`} key={project.number}><button className="project-main" onClick={() => setProjectOpen(projectOpen === index ? null : index)} aria-expanded={projectOpen === index}><span className="project-number mono">{project.number}</span><div><span className="project-category mono">{project.category}</span><h3>{project.title}</h3></div><span className="project-status mono">{project.status}<i>↗</i></span></button>{projectOpen === index && <div className="project-detail"><div><p>{project.description}</p><div className="tags">{project.tags.map((tag) => <span key={tag}>{tag}</span>)}</div></div><dl><div><dt>YEAR</dt><dd>{project.year}</dd></div><div><dt>TECHNICAL NOTES</dt><dd>{project.notes}</dd></div></dl></div>}</article>)}</div></section>

      <section className="map-section" id="map"><div className="section-heading compact"><p className="section-index mono">05 / RESEARCH MAP</p><h2>THE MAP</h2><p>One question, approached from three directions. Hover or focus a node to inspect it.</p></div><div className="research-map"><svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true"><path d="M50 50L22 28M50 50L78 28M50 50L50 82M22 28L8 14M22 28L8 40M22 28L27 8M22 28L30 48M78 28L92 14M78 28L92 40M78 28L72 8M78 28L72 49M78 28L91 64M50 82L31 92M50 82L50 96M50 82L72 92" /></svg>{mapNodes.map((node) => <button key={node.label} className={`map-node ${node.center ? "center" : ""}`} style={{ "--x": `${node.x}%`, "--y": `${node.y}%` } as CSSProperties} aria-label={`${node.label}: ${node.detail}`}><span>{node.label}</span><small>{node.detail}</small></button>)}</div></section>
